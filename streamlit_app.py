...
st.markdown('<p class="subtitle">Genera immagini & discuti con l\'IA</p>', unsafe_allow_html=True)
...
for message in st.session_state.chat_history:
    if message["role"] == "user":
        ...
        <div class="message-ai">
            <div class="bubble ai-bubble">{message['content']}</div>
        </div>
        """, unsafe_allow_html=True)
        if "generated_image" in message and message["generated_image"]:
...
if submit:
    ...
    # Generazione immagine Qwen-Image
    if prompt_text.strip() or caption_text:
        ...
        try:
            ...
            # Controlla se è path o b64
            if isinstance(result, str):
                generated_image = Image.open(result)
            else:
