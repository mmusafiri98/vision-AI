...
def format_web_search_for_prompt(query, search_type="web"):
    """Formate les résultats de recherche pour le prompt - VERSION GOOGLE UNIQUEMENT"""
...
def detect_search_intent(user_message):
    ...
    # Ordre de priorité: News → Wiki → Google (défaut)
...
with col1:
    ...
    st.write("- Recherche Google (puissante)")
    st.write("- Scraping de pages web")
    st.write("- Actualités Google News")
...
# -------------------------
with st.expander("⚙️ Configuration Google API (IMPORTANT)"):
    st.markdown("""
    ...
    **Comment obtenir les clés:**
    1. Google Custom Search: https://console.cloud.google.com/
    2. Créez un projet
    3. Activez "Custom Search API"
    4. Créez une clé API
    5. Créez un Search Engine sur: https://programmablesearchengine.google.com/
    ...
    **Statut actuel:**
    ...
    if GOOGLE_API_KEY:
        ...
        st.error("❌ Google API manquante - Recherche web désactivée")
...
if st.sidebar.button("🧪 Test Google Search"):
    ...
    if GOOGLE_API_KEY and GOOGLE_SEARCH_ENGINE_ID:
        ...
        st.sidebar.error("⚠️ Configurez Google API dans les Secrets !")
