...
if DB_AVAILABLE and st.session_state.user and st.session_state.user.get('id') != "guest":
    ...
    try:
        ...
        if convs:
            ...
            new_conv = db.create_conversation(user_id, "Conversation automatique")
            if new_conv:
...
if DB_AVAILABLE and st.session_state.user:
    st.markdown(
        f"<p style='text-align:center; color:#666;'>Connecté en tant que: <b>{st.session_state.user.get('email', 'Utilisateur')}</b></p>",
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Analyser cette image", type="primary"):
            ...
            if DB_AVAILABLE and conv_id:
                ...
                st.session_state.messages_memory.append({"role": "assistant_api_response", "content": response})
            st.rerun()
...
with chat_container:
    ...
    for msg in messages_to_display:
        role = "user" if msg["role"] in ["user","user_api_request"] else "assistant"
